...
//    println("try hello world".split(' '))
  ...
  fun algorithm24() {
    fun solution(N: Int, stages: IntArray): IntArray {
      val list = List(N) { index ->
        Pair(index + 1,
          if ((stages.count { index + 1 == it }.toDouble() / stages.count { index < it }.toDouble()).isNaN()) 0.0 else stages.count { index + 1 == it }
            .toDouble() / stages.count { index < it }.toDouble()
        )
  ...
  fun algorithm26() {
    fun solution(food: IntArray): String {
      val foodSettingRule = food.drop(1).mapIndexed { index, i -> index + 1 to i / 2 }.joinToString("") { it.first.toString().repeat(it.second) }
      return foodSettingRule + "0".repeat((food[0] / 2) + food[0] % 2) + foodSettingRule.reversed()
    }
  ...
  @Test
  fun algorithm28() {
    fun solution(k: Int, m: Int, score: IntArray) = score.sortedDescending().chunked(m).map { ints -> if (ints.size % m == 0) ints else emptyList() }.sumOf { (it.minOrNull() ?: 0) * m }

    println(solution(3, 4, intArrayOf(1, 2, 3, 1, 2, 3, 1))) // 8
    println(solution(4, 3, intArrayOf(4, 1, 2, 2, 4, 4, 4, 4, 1, 2, 4, 2))) // 33
  }
 ...
 *
  ...
  @Test
  fun algorithm27() {
    fun solution(number: Int, limit: Int, power: Int) = (1..number).map { (1..it).filter { mNumber -> it % mNumber == 0 }.size }.sumOf { if (it > limit) limit - 1 else it }
    ...
    println(solution(5, 4, 2)) // 10
    println(solution(10, 3, 2)) // 21
  }
}